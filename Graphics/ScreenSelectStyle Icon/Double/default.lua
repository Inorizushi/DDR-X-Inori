return Def.ActorFrame{
  Def.Sprite{
    InitCommand=function(s)
      ...
      if GAMESTATE:GetMasterPlayerNumber() == 'PlayerNumber_P1' then
        s:Load(THEME:GetPathG("","ScreenSelectStyle Icon/Double/Art (doubleres).png"));
      else
        s:Load(THEME:GetPathG("","ScreenSelectStyle Icon/Double/Art2 (doubleres).png"));
      end;
    end;
  ...
  Def.Sprite{
    InitCommand=cmd(xy,198,33);
    OnCommand=cmd(zoom,0;sleep,0.4;linear,0.05;zoom,1.1;linear,0.05;zoom,1);
    OffCommand=cmd(sleep,0.1;linear,0.05;zoomy,0;diffusealpha,0);
    GainFocusCommand=function(s)
      s:Load(THEME:GetPathG("","ScreenSelectStyle Icon/Double/Color (doubleres).png"));
    ...
    LoseFocusCommand=function(s)
      s:stoptweening()
      s:Load(THEME:GetPathG("","ScreenSelectStyle Icon/Double/Gray (doubleres).png"));
      s:stopeffect()
    end;
  };
  Def.ActorFrame{
    InitCommand=cmd(xy,122,-96);
  	...
  	PlayCommand=function(self)
  		...
  		SOUND:PlayAnnouncer( "ss comment/double" )
  		end;
  	end;
    LoadActor( "../nfofill" )..{
  	};
